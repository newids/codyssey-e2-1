...
- 정적 우선 원칙: 기본은 런타임 LLM 호출 없음(적응형 해설은 데이터셋 `clues`의 클라이언트 렌더링). 예외(2026-08-20, 전략 v3.0 M1): AI 가이드 해설 1경로에 한해 런타임 LLM 호출 허용 — 규칙 기반 폴백 상시 유지, 빌드 산출물에 API 키 미포함, 호출 상한 적용.
